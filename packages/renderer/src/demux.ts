 ...
 */
export interface VideoSampleRef {
  offset: number;
  size: number;
  timestampUs: number;
  durationUs: number;
  /** Decodable without any earlier sample. */
  key: boolean;
}
...
function readPictureTrack(view: DataView, trak: BoxRef): SourceVideoTrack | null {
  ...
  const presentation = decodeTimes.map((at, index) => at + (composition[index] ?? 0));
  ...
  const samples: VideoSampleRef[] = order.map((index, position) => {
    const next = order[position + 1];
    const span = next === undefined ? deltas[index]! : presentation[next]! - presentation[index]!;
    return {
      offset: offsets[index]!,
      size: sizes[index]!,
      timestampUs: toUs(presentation[index]!),
      durationUs: Math.max(1, toUs(Math.max(1, span))),
      key: sync[index] ?? false,
    };
  });
...
export async function readVideoWeights(file: Blob): Promise<VideoWeights | null> {
  ...
  const rate = await readVideoFrameRate(file);
  const hopUs = rate
    ? Math.round((1_000_000 * rate.frameRate.den) / rate.frameRate.num)
    : Math.round(track.durationUs / track.samples.length);
  if (!Number.isFinite(hopUs) || hopUs <= 0) return null;

  return {
    hopUs,
    sizes: track.samples.map((sample) => sample.size),
    times: track.samples.map((sample) => sample.timestampUs),
    sync: track.samples.map((sample) => sample.key),
  };
}
