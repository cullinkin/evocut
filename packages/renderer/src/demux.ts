 ...
 * motion", and the honest way to get motion is to decode frames and difference them. On a
 * phone, against a multi-gigabyte recording, that is not available: a seek costs most of a
 * second and there are two decoders already in use for the preview. The filmstrip pass
 * tried it — six hundred seeks per source — and the cost was whole seconds of frozen
 * interface, measured off a screen recording of a real session.
 *
 ...
 * mean absolute difference between decoded frames.
 *
 * The whole curve, per frame, for a recording of any length, comes out of `moov` — the same
 * few hundred kilobytes the audio index is already read from. Nothing is decoded and no
 * frame is ever seeked to.
 *
 * ## What it is not
 *
 ...
 * about movement at all — they are a whole picture, not a difference — so they are bridged
 * rather than measured. It is a legible instrument for *where something happens*, in the
 * way the audio envelope is, and it is used for exactly that.
 */
export interface VideoWeights {
  ...
  times: number[];
  /** True where the frame is a keyframe, whose size says nothing about movement. */
  sync: boolean[];
}
...
export async function readVideoWeights(file: Blob): Promise<VideoWeights | null> {
  ...
  for (const trak of childBoxes(view, 0, bytes.byteLength)) {
    if (trak.type !== 'trak') continue;
    const weights = readVideoTrackWeights(view, trak);
    if (weights) return weights;
  }
  return null;
}

function readVideoTrackWeights(view: DataView, trak: BoxRef): VideoWeights | null {
  ...
  if (deltas.length !== sizes.length) return null;

  const offsets = readCompositionOffsets(view, stbl, sizes.length);
  const sync = readSyncSamples(view, stbl, sizes.length);

  /*
    Presentation order, not decode order.

    A container stores frames in the order a decoder needs them, which with B-frames is not
    the order they are shown in. Left alone, that would smear the curve by a few frames
    around every reordered group — small, but this is an instrument someone aims a cut
    with, and being a few frames out is the exact complaint it exists to answer.
  */
  const order = sizes.map((_, index) => index);
  let decode = 0;
  const decodeTimes = deltas.map((delta) => {
    const at = decode;
    decode += delta;
    return at;
  });
  const presentation = decodeTimes.map((at, index) => at + (offsets[index] ?? 0));
  order.sort((a, b) => presentation[a]! - presentation[b]!);

  const toUs = (ticks: number) => Math.round((ticks * 1_000_000) / timescale);
  const histogram = sampleDurationHistogram(view, stbl);
  const hopUs = histogram.commonest > 0 ? toUs(histogram.commonest) : toUs(decode / sizes.length);
  if (!Number.isFinite(hopUs) || hopUs <= 0) return null;

  return {
    hopUs,
    sizes: order.map((index) => sizes[index]!),
    times: order.map((index) => toUs(presentation[index]!)),
    sync: order.map((index) => sync[index] ?? false),
  };
}
