...
export interface FrameWeights {
  ...
  /** Non-zero where the frame is a keyframe. */
...
export function analyzePicture(weights: FrameWeights): PictureSignals | null {
  ...
  let allIntra = true;
  for (let index = 0; index < sync.length && allIntra; index += 1) allIntra = sync[index] !== 0;
  const measured = allIntra ? Array.from(sizes) : bridgeKeyframes(sizes, sync);
 ...
 */
function bridgeKeyframes(sizes: ArrayLike<number>, sync: ArrayLike<number>): number[] {
  const out = Array.from(sizes);
